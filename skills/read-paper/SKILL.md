...
21. **(2026-09-18)公式不要写进 chat——用户的 VSCode 插件不渲染 LaTeX。** 他原话：「我在 vscode 插件中看不到你写的公式，tmd 全部都是 latex 原文。」
    - 工作区 `CLAUDE.md` 本来就写着「数学重的解释要写进 note.html，不要只写进 chat」，**agent 违反了好几轮才被发现**。
    - **规矩**：chat 里只写能用纯文字说清的部分；任何 `$...$`、任何推导、任何符号密集的表，一律进 `note.html` 的 Q 块或讨论存稿。
    - chat 里可以写的数学：**已经算出来的数**（「70,000 倍」「Spearman −0.87」）、**变量名的白话**（「那个错标率」）。**不能写的**：公式本身。

22. **(2026-09-18)讨论存稿与主线文件分家。** 用户问「开一个 html 讨论，还是直接在研究主线中讨论」。**答案是分开**，因为职责不同：
    - **主线** `research-mainline.html` = **结论**。用户要求过它「准确简洁」，塞进过程会把「已定」和「正在想」混掉。
    - **讨论存稿** = **过程**。可以乱、可以推翻自己、可以留死胡同。**想定了的才搬进主线。**
    - 工作区已有先例：`70-search-planning/data-quality-nail-2026-09-14.html`。当前实例：`discussion-information-transfer.html`。
    - **讨论存稿的每一块必须挂标签**：文献事实 / 用户的判断 / agent 的构造 / 未决。**agent 的构造不许伪装成文献事实。**
