...
## 13. v1.9.3(2026-09-18):ML 灯暗的论文要有速读块

用户裁定：**「对于 ML 这一个灯没有亮的情况，结论的展示要尽可能的提前，并加粗高亮……我不想在这种论文上浪费太多的时间」**。

**触发**：一屏卡三镜行判 **ML 空**。判据照 §三镜原样：只有「我们观察到 X 和 Y 一起变」，没有证明、没有能隔离因果的消融。

**位置：一屏卡之前，页面第一眼。** 这一条是实测出来的——先放在卡片和 Q 索引之间，探针量出 `tldrTop=2188 px`，意味着用户要滚过整张卡片图才看得到，等于没提前。挪到 `<!-- ① 一屏卡 -->` 注释之前后 `tldrTop=262`。目录里单开一组「先读这个」置顶。

**固定五段，顺序不能换**：

| 段 | 内容 | 硬要求 |
|---|---|---|
| ① 一句话裁决 | 它**测出了什么**、**解释不了什么** | 整段加粗关键句；说清「只能查表，不能外推」 |
| ② 能带走的 | 编号列 | **每条必须是数字或可执行规则**。「多样性比数量重要」不算，「$\|r\|=0.942$–$0.991$ 对 $-0.62$」才算 |
| ③ 不能带走的 | 逐条 | 写清**外推到哪里就错**：指数不是跨任务常数、阈值不普适、哪条预测作者自己没验、哪些数不可比 |
| ④ ML 灯为什么暗 | 一段 | **必须配作者自己承认缺口的原句**。这类论文的作者通常在局限段直接写了 |
| ⑤ 读到这里就可以走了 | 一句 | 说明下面的课**在什么条件下**才值得看（通常是「你要复制这套实验设计时」） |

**讲义照常写完，不许偷工。** 速读块是入口不是替代：用户要的是不浪费时间，不是少一份材料。

**反过来也成立**：ML 灯**亮**的论文**不加**速读块——那种论文的价值在推导里，把结论抽到最前面反而鼓励跳读。

**CSS**（`.tldr`，红框 + 深红标题条）与实例见 `72-data-quality/2410.18647-*/note.html`。

**为什么这条规矩值钱**：用户对 2410 的判断是「这只是一个迁移，得奖主要靠的是工作量和确实解决了很多问题，**但他无法解释任何东西**」。工作量换来的是**可查表的数字**——4 万条示范买到「每对 50 条、32 对够用」这样的预算公式。那就让它以数字的形式在三分钟内被取走，而不是包在八课里。

### 13.1 T3 / T4 的第 0 课就是结论（2026-09-18 当天追加）

用户在速读块交付后立刻追问：**「为什么结论不应该在第 0 课就出现？我还要去看她的 exp？」** 顶部速读块解决的是「打开页面第一眼」，但**讲义轨道本身**仍然是「为什么读 → 实验 → 曲线 → 结论」，用户进了讲义还是得走完实验。

**改法：T3 / T4 的第 0 课改名为「这篇的结论，以及你怎么用」，固定四段。**

| 段 | 内容 | 硬要求 |
|---|---|---|
| 0.1 | 几条结论，每条一个 `.exbox` | **每条必须是数字或可执行规则** |
| 0.2 | 把它变成你自己的预算 / 配方 | **这一段放交互组件**，让用户拖出自己的数，而不是读别人的数 |
| 0.3 | 什么时候这套数**不**能用 | 逐条，写清前提 |
| 0.4 | 「这一课到此为止」+ 三个指路 | 数怎么来的 / 为什么不能外推 / 实验设计各指一课 |

元信息（为什么读这篇、本笔记的行话、路线存稿原文）**折叠进第 0 课末尾的 `<details>`**——不删，但不挡路。

**实验课排在结论之后，作为佐证，不作为通路。** 讲义顺序既不是论文的顺序也不是推理的顺序，是**先给答案再给理由**。

**ML 灯亮的论文不适用**：那种论文的结论离开推导不成立，第 0 课仍写「为什么读」。

**实例**：`72-data-quality/2410.18647-*/note.html` 第 0 课（四条结论 + 采集预算计算器）与第 4 课（三个数的出处 + 每对/每环境切换组件）。

### 13.2 顺带记一条方法：T3 论文的讲义价值，往往在「把它自己的数交叉一次」

2410 的第 4 课重写时，把论文自己给的**四个平台期**放在一起除了一次，发现论文说的不变量（每个环境-物体对 50 条）在它自己的第四个设定上差了四倍，而换成**每个环境 50 条**四行全部对上。论文两处措辞本来就不一致（Sec 4.3 写 per environment-object pair，Sec 5 写 per environment），只是在它的推荐配方里两者重合，所以没人注意。

**这类发现不需要新实验，只需要把论文分散在几节里的数放进同一张表。** T3 论文机制空，但数多，**交叉它自己的数是这类论文唯一能产出新东西的读法**，比复述它的结论有价值得多。写讲义时主动找这种机会。
