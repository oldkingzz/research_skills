...
> **v1.6(2026-09-04)适用范围**:不再生成独立 paper-card.md。§1 脊椎、§4 批判(改叫"边界")、§5 裁决 → `note.html` **一屏卡左栏**;§2 K 件事、§3 承重数学 → **夯实 tab 提取卡的答案面**。"1 屏内"约束仍适用于一屏卡。右栏由用户自己写,agent 不碰。**v1.7(2026-09-12)**:本文件"不做跨 paper connect"的规则作废,改为"连但每条标出处";一屏卡另加脉络 / 三镜 / 钉子候选三行,agent 全自动填,见 note_schema_v2.md §9。
